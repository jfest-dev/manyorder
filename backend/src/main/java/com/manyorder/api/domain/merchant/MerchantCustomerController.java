...
public class MerchantCustomerController {
    ...
    /**
     * Edit a customer's live details (full-record replace). Owner-only. 200 with
     * the updated customer; 409 if the new phone/email belongs to another
     * customer; 404 if not in this store. Past orders keep their snapshots.
     */
    @PutMapping("/{customerId}")
    public CustomerResponse updateCustomer(@PathVariable Long storeId, @PathVariable Long customerId,
                                           @Valid @RequestBody UpdateCustomerRequest request,
                                           Authentication authentication) {
        User user = currentUserService.require(authentication);
        Merchant merchant = storeAccessService.requireOwnedStore(user, storeId);
        return customerService.updateCustomer(merchant, customerId, request);
    }
