...
import { isOrderable, maxOrderQuantity, lowStockRemaining } from './storefrontTypes';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%', background: 'white' }}>
      ...
      <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        ) : lowStockRemaining(product) !== null ? (
          <div style={{ fontSize: '12px', fontWeight: 600, color: '#C2410C', background: '#FFEDD5', padding: '8px 10px', borderRadius: '8px' }}>
            Only {lowStockRemaining(product)} left
          </div>
        ) : null}
        ...
        {!preview && (
          <div style={{ marginTop: '4px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
              <QuantityStepper quantity={qty} onChange={setQty} min={1} max={maxOrderQuantity(product)} maxHint size="md" disabled={!orderable} />
              <button
                disabled={!canSubmit}
                onClick={submit}
                style={{
