...
import type { PublicStoreResponse, GuestCheckoutResult } from '../../lib/api';
...
function whatsappUrl(result: GuestCheckoutResult, store: PublicStoreResponse): string | null {
  ...
  lines.push('');
  lines.push(`Order #${result.orderId}`);
  result.items.forEach((it) => lines.push(`• ${it.quantity} × ${it.productName}: ${formatMoney(it.subtotal, c)}`));
  lines.push('');
  ...
  lines.push(`Total: ${formatMoney(result.totalAmount, c)}`);
  lines.push('');
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%', background: '#F3F4F6' }}>
      <div style={{ padding: '32px 16px 20px', textAlign: 'center', background: 'white', borderBottom: '1px solid var(--border-subtle)' }}>
        ...
        <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginTop: '6px' }}>
          Order #{result.orderId} · {result.storeName}
        </p>
      </div>
      ...
      <div style={{ flex: 1, padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <section style={{ background: 'white', border: '1px solid var(--border-subtle)', borderRadius: '12px', padding: '14px' }}>
          ...
          <div style={{ borderTop: '1px solid var(--border-subtle)', marginTop: '8px', paddingTop: '8px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <Row label="Total" value={formatMoney(result.totalAmount, currency)} bold />
          </div>
        </section>
